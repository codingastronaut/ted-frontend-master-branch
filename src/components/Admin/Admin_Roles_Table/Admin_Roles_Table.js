...
const Admin_Roles_Table = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row, index) => (
        <div size="middle">
          <a onClick={() => navigate(`/adminrolesform/${row.id}`)}>
            <EditIcon />
          </a>
  ...
  const handleChange = (e) => {
    if (e.target.value.length) {
      ...
      let filterObj = mainPost.filter(
        (el) => el.name.includes(value) || el.email.includes(value)
      );
...
export default Admin_Roles_Table;
